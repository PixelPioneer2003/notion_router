...
const Testimonial = ({ reviews, setReviews, isLoggedIn }) => {
  ...
  const handleUpdateProblemSubmit = async (updatedProblem) => {
    try {
      // Send the updated problem data to the server
      console.log(updatedProblem);
      console.log("My "+updatedProblem._id);
      const response = await axios.put(
        `http://localhost:4002/api/v1/admin/updateQuestion/${updatedProblem._id}`, // Assuming you use a PUT request and the endpoint accepts the problem ID
        updatedProblem
      );
      console.log("Updated");
      ...
      // Hide the update form and reset the current problem
      setShowUpdateForm(false);
      setCurrentProblem(null);
    } catch (error) {
